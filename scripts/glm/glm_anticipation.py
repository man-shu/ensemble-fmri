...
if __name__ == "__main__":
    ...
    three_mm_dir = os.path.join(ROOT, "data", "aomic_anticipation", "3mm")
    os.makedirs(three_mm_dir, exist_ok=True)

    # design matrix plots dir
    design_matrix_dir = os.path.join(
        ROOT, "data", "aomic_anticipation", "design_matrices"
    )
    ...
    fmri = sorted(
        glob(
            os.path.join(
                ...
                "sub-*",
                "func",
                "*task-anticipation*MNI*bold.nii.gz",
            )
        )
    )
    ...
    events = [
        glob(
            os.path.join(
                source_dir, sub, "func", "*task-anticipation*events.tsv"
            )
        )[0]
        for sub in subjects
    ]
    masks = [
        glob(
            os.path.join(
                ...
                sub,
                "func",
                "*task-anticipation*MNI*brain_mask.nii.gz",
            )
        )[0]
        for sub in subjects
    ]
