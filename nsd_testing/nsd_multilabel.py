...
def cross_val(
    ...
    cv = ShuffleSplit(test_size=0.20, random_state=0, n_splits=5)
    ...
    if classifier == "LogisticRegression":
        ...
        clf = MultiOutputClassifier(LinearSVC(dual="auto"), n_jobs=10)
    else:
        raise ValueError("Classifier not implemented.")
    if classifier == "MultiLabelSVC":
        dummy_clf = MultiOutputClassifier(DummyClassifier(), n_jobs=10)
    else:
...
if __name__ == "__main__":
    ...
    all_results = Parallel(n_jobs=5, verbose=11, backend="loky")(
        delayed(cross_val)(
            subject,
            data,
            classifier,
