...
if __name__ == "__main__":
    ...
    # hand conditions
    main_conditions = ["pride", "anger", "joy", "neutral", "contempt"]
    ...
    fmri = sorted(
        glob(
            os.path.join(
                ...
                "sub-*",
                "func",
                "*task-faces*MNI*bold.nii.gz",
            )
        )
    )
    subjects = [f.split(os.sep)[11] for f in fmri]
    events = [
        glob(os.path.join(source_dir, sub, "func", "*task-faces*events.tsv"))[
            0
        ]
        for sub in subjects
    ]
    masks = [
        glob(
            os.path.join(
                ...
                sub,
                "func",
                "*task-faces*MNI*brain_mask.nii.gz",
            )
        )[0]
        for sub in subjects
    ]
