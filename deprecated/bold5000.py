...
for fold in ["fold2", "fold3", "fold4"]:
    data_resolution = fold  # or 1_5mm
    nifti_dir = os.path.join(data_dir, data_resolution)
    classifiers = ["LinearSVC", "RandomForest"]

    # output results path
    start_time = time.strftime("%Y%m%d-%H%M%S")
    results_dir = f"{dataset}_{data_resolution}_results_{start_time}"
    ...
    data = dict(responses=[], conditions=[], subjects=[])
    # load data
    print("Loading data...")
    for subject in tqdm(subjects):
        try:
            response = image.load_img(
                os.path.join(nifti_dir, f"{subject}.nii.gz")
            )
            print(response.shape)
            # trial conditions
            conditions = pd.read_csv(
                os.path.join(nifti_dir, f"{subject}_labels.csv"),
                header=None,
            )
            conditions = conditions[0].values
            # run labels
            # runs = pd.read_csv(
            #     os.path.join(nifti_dir, f"{subject}_runs.csv"),
            #     header=None,
            # )
            # runs = runs[0].values
    ...
    # concatenate data
    for dat in ["responses", "conditions", "subjects"]:
    ...
    def decode(
        ...
        results.to_pickle(
            os.path.join(
                results_dir, f"results_clf_{classifier}_{subject}.pkl"
            )
        )
        return results
