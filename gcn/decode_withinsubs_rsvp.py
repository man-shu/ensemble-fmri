...
if __name__ == "__main__":
    ...
    split_ts_withinsubs = os.path.join(ROOT, "data", "rsvp", "split_time_series_difumo_withinsubs")
    os.makedirs(split_ts_withinsubs, exist_ok=True)
    ...
    for subject, session in tqdm(subject_session):
        ...
        for file in os.listdir(split_ts_withinsubs):
            os.remove(os.path.join(split_ts_withinsubs, file))
        ...
        for fmri_file, confound_file, events_file in zip(fmri, confounds, events):
            ...
            # check existence of split time series
            split_time_series_files = glob(os.path.join(split_ts, f"{subject}_{run}*.npy"))
            if len(split_time_series_files) > 0:
                print(f"Loading pre-existing split time series for {subject}_{run}")
                for split_time_series_file in split_time_series_files:
                    time_point = np.load(split_time_series_file)
                    filename = split_time_series_file.split("/")[-1]
                    np.save(os.path.join(split_ts_withinsubs, filename), time_point)
                    label = filename.split("_")[2]
                    encoded_labels_patch = {name.split("_")[0]: i for i, name in enumerate(main_conditions)}
                    labels_df.append({"label": encoded_labels_patch[label], "filename": filename})
                    X.append(time_point.squeeze())
                continue
        ...
        labels_df.to_csv(os.path.join(split_ts_withinsubs, "labels.csv"), index=False)
        ...
        accuracies.append({"subject": subject, "accuracy": correct})
